...
class HBS_CL_CLAIM extends  BaseModelDB2
{
    ...
    public function getCsrRemarkAllAttribute(){
        $htm = "";
        foreach ($this->HBS_CL_LINE as $key => $value) {
            if($value->HBS_CL_LINE_REJ->count() > 0){
                $htm .=  \Carbon\Carbon::parse($value->incur_date_from)->format('d/m/Y')  ." - "  .\Carbon\Carbon::parse($value->incur_date_to)->format('d/m/Y') . "\n";
            }
            foreach($value->HBS_CL_LINE_REJ as $key2 => $value2){
                $htm .= "từ chối số tiền " .$value2->rej_amt . " vì: " .($value2->HBS_SY_SYS_CODE_LANG->code_desc_vn ?  $value2->HBS_SY_SYS_CODE_LANG->code_desc_vn : $value2->HBS_SY_SYS_CODE_LANG->code_desc) ."\n";
            }
        }
        $htm .=  $this->remark;
        return $htm;
