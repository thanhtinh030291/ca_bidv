...
class HBS_MR_MEMBER extends  BaseModelDB2
{
    ...
    public function MR_POLICYHOLDER()
    {
        return $this->belongsTo('App\HBS_MR_POLICYHOLDER','poho_oid');
    }
    public function getClaimLineAttribute()
    {
        $CL_LINE = HBS_CL_LINE::where('memb_oid',$this->memb_oid)->with('PD_BEN_HEAD')->with('RT_DIAGNOSIS')->with('HBS_CL_CLAIM')->where('REV_DATE', null)->get();
        return $CL_LINE;
    }
